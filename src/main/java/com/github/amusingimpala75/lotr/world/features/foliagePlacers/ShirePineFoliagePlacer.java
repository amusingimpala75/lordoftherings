...
public class ShirePineFoliagePlacer extends FoliagePlacer {
    public static Codec<ShirePineFoliagePlacer> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, instance.stable(ShirePineFoliagePlacer::new)));
    public ShirePineFoliagePlacer(UniformIntDistribution radius, UniformIntDistribution offset) {
        super(radius, offset);
    }
    //public ShirePineFoliagePlacer(int r, int rRand) {
    //    super(r, rRand, LOTRFoliagePlacers.SHIRE_PINE_FOLIAGE);
    //}

    //public <T> ShirePineFoliagePlacer(Dynamic<T> dyn) {
    ...
    public void func_225571_a_(ModifiableTestableWorld world, Random rand, TreeFeatureConfig config, int leafTop, int leafBottom, int maxLeafWidth, BlockPos base, Set<BlockPos> leaves) {
        ...
        for(int y = leafTop; y >= leafBottom; --y) {
            ...
            if (layerWidth >= nextMaxLayerWidth) {
                ...
                nextMaxLayerWidth = Math.min(nextMaxLayerWidth + 1, maxLeafWidth);
            } else {
                ++layerWidth;
            }
        }
    ...
    protected boolean func_225572_a_(Random rand, int leafTopY, int x, int currentLayerY, int z, int layerWidth) {
        return Math.abs(x) == layerWidth && Math.abs(z) == layerWidth && layerWidth > 0;
    }
    ...
    protected FoliagePlacerType<?> getType() {
        return ModDefaultBiomeFeatures.SHIRE_PINE_FOLIAGE;
    }
