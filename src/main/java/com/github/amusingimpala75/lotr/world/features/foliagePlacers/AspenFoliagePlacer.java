...
public class AspenFoliagePlacer extends FoliagePlacer {
    public static Codec<AspenFoliagePlacer> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, instance.stable(AspenFoliagePlacer::new)));

    public AspenFoliagePlacer(UniformIntDistribution radius, UniformIntDistribution offset) {
        super(radius,offset);
    }

    @Override
    protected FoliagePlacerType<?> getType() {
        return ModDefaultBiomeFeatures.ASPEN_FOLIAGE;
    }
    ...
    public void func_225571_a_(ModifiableTestableWorld world, Random rand, TreeFeatureConfig config, int leafTop, int leafBottom, int maxLeafWidth, BlockPos base, Set<BlockPos> leaves) {
        for(int y = leafTop; y >= leafBottom; --y) {
            ...
            for(int b = 0; b < branches; ++b) {
                BlockPos.Mutable movingPos = (new BlockPos.Mutable(base.getX(), base.getY(), base.getZ())).move(0, y, 0);
                ...
                for(int l = 0; l < length && Math.abs(origX - movingPos.getX()) <= layerWidth && Math.abs(origZ - movingPos.getZ()) <= layerWidth; ++l) {
                    this.method_23450(world, rand, movingPos, config, leaves);
                    Direction randDir = Direction.Type.HORIZONTAL.random(rand);
    ...
    /*protected void func_227385_a_(ModifiableTestableWorld arg, Random random, TreeFeatureConfig arg2, int i, BlockPos arg3, int j, int k, Set<BlockPos> set) {
