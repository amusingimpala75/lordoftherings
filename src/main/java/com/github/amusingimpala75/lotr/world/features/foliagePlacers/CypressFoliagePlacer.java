...
public class CypressFoliagePlacer extends FoliagePlacer {
    public static Codec<CypressFoliagePlacer> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, instance.stable(CypressFoliagePlacer::new)));
    public CypressFoliagePlacer(UniformIntDistribution radius, UniformIntDistribution offset) {
        super(radius, offset);
    }
    //public CypressFoliagePlacer(int r, int rRand) {
    //    super(r, rRand, LOTRFoliagePlacers.CYPRESS_FOLIAGE);
    //}

    //public <T> CypressFoliagePlacer(Dynamic<T> dyn) {
    ...
    public void func_225571_a_(ModifiableTestableWorld world, Random rand, TreeFeatureConfig config, int leafTop, int leafBottom, int maxLeafWidth, BlockPos base, Set<BlockPos> leaves) {
        int topCrossY = leafTop - 2;
        int bottomCrossesHighestY = leafBottom + rand.nextInt(3);
        int layerWidth = 0;
        for (int y = leafTop; y >= leafBottom; y--) {
            ...
            func_227384_a_(world, rand, config, leafTop, base, y, layerWidth, leaves);
            if (y == topCrossY || y <= bottomCrossesHighestY) {
                func_227384_a_(world, rand, config, leafTop, base.west(), y, layerWidth, leaves);
                func_227384_a_(world, rand, config, leafTop, base.east(), y, layerWidth, leaves);
                func_227384_a_(world, rand, config, leafTop, base.north(), y, layerWidth, leaves);
                func_227384_a_(world, rand, config, leafTop, base.south(), y, layerWidth, leaves);
            }
        }
    ...
    protected FoliagePlacerType<?> getType() {
        return ModDefaultBiomeFeatures.CYPRESS_FOLIAGE;
    }
